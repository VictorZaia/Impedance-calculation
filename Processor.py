...
class Processor:
    ...
    def resistance_eq(self, r, p_acous_pa, r_tot_plate, chi_tot_plate, chi_cavity, sigma, M):
        """Solve the resistance equation."""
        B = (1 - sigma**2) / (sigma * self._Environment.c)
        impedance_magnitude = abs(r + 1j * (chi_tot_plate + chi_cavity))
        r_airflow = self.compute_resistance_tangencial_airflow(sigma, M)

        return r - (r_tot_plate + B * p_acous_pa / (self.Environment.rho * self.Environment.c * sigma * impedance_magnitude) + r_airflow)
